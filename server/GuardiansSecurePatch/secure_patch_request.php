<?php
// U-CREW Guardians güvenli yama bileti üretimi.
// Bu dosyayı API klasörüne koyun: /api/secure_patch_request.php

ob_start();
require_once __DIR__ . '/../includes/bootstrap.php';
require_once __DIR__ . '/auth_v2.php';

function guardians_secure_out($data, $code = 200) {
...
if ($_SERVER['REQUEST_METHOD'] === 'OPTIONS') {
    guardians_secure_out(array('status' => 'ok'));
}

if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    guardians_secure_out(array(
        'status' => 'error',
        'message' => 'Yalnızca POST isteği kabul edilir.'
    ), 405);
}

function guardians_secure_token() {
...
function guardians_secure_user() {
    $token = guardians_secure_token();
...
try {
    ...
    $gameQuery = $pdo->prepare(
        'SELECT id, slug FROM games WHERE LOWER(slug)=LOWER(?) LIMIT 1'
    );
    ...
    if (!$game) {
        guardians_secure_out(array(
            'status' => 'error',
            'message' => 'Guardians oyunu U-CREW panelinde bulunamadı.'
        ), 404);
    }
    ...
    $patchQuery = $pdo->prepare(
        "SELECT id, version, file_name, file_size, sha256,
    ...
    $patchQuery->execute(array((int)$game['id']));
    $patch = $patchQuery->fetch(PDO::FETCH_ASSOC);

    if (!$patch) {
        guardians_secure_out(array(
            'status' => 'error',
            'message' => 'Guardians için etkin güvenli yama paketi bulunamadı.'
        ), 404);
    }
    ...
    error_log('Guardians secure patch request: ' . $error->getMessage());
